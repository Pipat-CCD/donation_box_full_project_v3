...
คุณสมบัติหลัก:
- ใช้ CSV ได้ทันที และเชื่อม Google Sheets/Drive ได้เมื่อใส่ Streamlit Secrets
- ระบบล็อกอินหลายผู้ใช้ พร้อมบันทึกชื่อเจ้าหน้าที่อัตโนมัติ
- เพิ่ม/ลบ/แก้ไข รายการกล่อง
- อัปโหลดข้อมูลกล่องทั้งหมดหรือบางรายการด้วยไฟล์ CSV ตามแพทเทิร์น
...
BOXES_CSV = DATA_DIR / "boxes_master.csv"
COLLECTIONS_CSV = DATA_DIR / "collections.csv"
MISSING_CSV = DATA_DIR / "missing_reports.csv"
...
def read_csv_df(path: Path, cols: List[str]) -> pd.DataFrame:
...
def backup_df_to_drive(sheet_name: str, df: pd.DataFrame):
    # สำรองข้อมูลเป็น CSV ใน Google Drive เพื่อให้ข้อมูลไม่อยู่เฉพาะเครื่อง/มือถือ
    if not has_drive_config():
        return
    data = df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8-sig")
    filename = f"backup_{sheet_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    upload_bytes_to_drive(data, filename, "text/csv")
        ...
        try:
            ...
            try:
                ...
                if fallback_path.exists():
                    seed = pd.read_csv(fallback_path, encoding="utf-8-sig").fillna("")
            ...
            st.warning(f"อ่าน Google Sheets ไม่สำเร็จ จึงใช้ไฟล์ CSV แทน: {e}")
    return read_csv_df(fallback_path, cols)
...
def save_sheet_df(sheet_name: str, path: Path, df: pd.DataFrame, cols: List[str], backup=True):
    ...
    if has_google_config():
        try:
            ...
            st.warning(f"บันทึก Google Sheets ไม่สำเร็จ จึงบันทึกลง CSV แทน: {e}")
...
def load_boxes_cached() -> pd.DataFrame:
    return normalize_boxes(get_sheet_df("boxes", BOXES_CSV, BOX_COLUMNS))


@st.cache_data(ttl=20, show_spinner=False)
def load_collections_cached() -> pd.DataFrame:
    return get_sheet_df("collections", COLLECTIONS_CSV, COLLECTION_COLUMNS)


@st.cache_data(ttl=20, show_spinner=False)
def load_missing_cached() -> pd.DataFrame:
    return get_sheet_df("missing_reports", MISSING_CSV, MISSING_COLUMNS)


@st.cache_data(ttl=20, show_spinner=False)
def load_users_cached() -> pd.DataFrame:
    users = get_sheet_df("users", USERS_CSV, USER_COLUMNS)
    if len(users) == 0:
        users = pd.DataFrame([{
        ...
        save_sheet_df("users", USERS_CSV, users, USER_COLUMNS, backup=False)
    return users
...
def save_boxes(df: pd.DataFrame):
    clear_and_save(); save_sheet_df("boxes", BOXES_CSV, normalize_boxes(df), BOX_COLUMNS)


def save_collections(df: pd.DataFrame):
    clear_and_save(); save_sheet_df("collections", COLLECTIONS_CSV, df, COLLECTION_COLUMNS)


def save_missing(df: pd.DataFrame):
    clear_and_save(); save_sheet_df("missing_reports", MISSING_CSV, df, MISSING_COLUMNS)


def save_users(df: pd.DataFrame):
    clear_and_save(); save_sheet_df("users", USERS_CSV, df, USER_COLUMNS)
...
def page_dashboard(boxes, collections, missing):
    ...
    c7.metric("รอบ 3 เดือน", int((boxes["cycle_months"] == 3).sum()))
    c8.metric("รอบ 6 เดือน", int((boxes["cycle_months"] == 6).sum()))
...
def page_boxes(boxes):
    ...
    for _, row in df.head(120).iterrows():
        ...
        <span class='badge {color}'>{text}</span> <span class='badge gray'>{row.get('status','')}</span>
        </div>
    ...
    st.download_button("⬇️ ดาวน์โหลดรายการที่กรองแล้ว CSV", df.to_csv(index=False, encoding="utf-8-sig"), "filtered_boxes.csv", "text/csv")
...
def page_import(boxes):
    st.title("📥 อัปโหลดรายการกล่องตามแพทเทิร์น")
    st.markdown("""
    ...
    st.download_button("⬇️ ดาวน์โหลดแพทเทิร์น CSV เปล่า", template.to_csv(index=False, encoding="utf-8-sig"), "box_upload_template.csv", "text/csv")
    st.download_button("⬇️ ดาวน์โหลดตัวอย่าง CSV", sample.to_csv(index=False, encoding="utf-8-sig"), "box_upload_sample.csv", "text/csv")
    uploaded = st.file_uploader("เลือกไฟล์ CSV รายการกล่อง", type=["csv"])
...
def page_reports(boxes, collections, missing, users):
    ...
    with tab1:
        ...
        st.download_button("⬇️ ดาวน์โหลดประวัติการเก็บ", collections.to_csv(index=False, encoding="utf-8-sig"), "collections.csv", "text/csv")
    with tab2:
        ...
        st.download_button("⬇️ ดาวน์โหลดแจ้งกล่องหาย", missing.to_csv(index=False, encoding="utf-8-sig"), "missing_reports.csv", "text/csv")
    with tab3:
        view_users = users.drop(columns=["password"], errors="ignore")
        st.dataframe(view_users, use_container_width=True, hide_index=True)
    with tab4:
        st.download_button("⬇️ ดาวน์โหลดรายการกล่องทั้งหมด", boxes.to_csv(index=False, encoding="utf-8-sig"), "boxes.csv", "text/csv")
        st.download_button("⬇️ ดาวน์โหลดประวัติการเก็บทั้งหมด", collections.to_csv(index=False, encoding="utf-8-sig"), "collections.csv", "text/csv")
        st.download_button("⬇️ ดาวน์โหลดรายงานกล่องหายทั้งหมด", missing.to_csv(index=False, encoding="utf-8-sig"), "missing_reports.csv", "text/csv")
...
def page_users(users):
    ...
    with tab_download:
        ...
        st.download_button("⬇️ ดาวน์โหลดบัญชีผู้ใช้แบบไม่แสดงรหัสผ่าน", safe_users.to_csv(index=False, encoding="utf-8-sig"), "users_no_password.csv", "text/csv")
...
def main():
    ...
    with st.sidebar:
        ...
        st.write("Google Sheets + Drive" if has_google_config() else "CSV Local")
